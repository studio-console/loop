...
1. **studio_project.py split into the studio_console/ package** (2026-08-13):
...
2. **Project moved into subfolder** (2026-08-12):
...
3. **Font set to 17px Avenir** (2026-08-12):
...
4. **GUI visual polish** (DeepSeek, 2026-08-12):
...
5. **stk/fdr rename** (DeepSeek, 2026-08-12):
...
6. **CLAUDE.md + CURRENT.md added** for AI handoff efficiency

## Known / pending

- Test stacks 11–14 get wiped by headless smoke test (`SaveShow` overwrites files) — re-seed manually after any headless run
- **File split is done** (see Recent changes #1) — not yet pushed to origin (26 commits ahead)
- Deliberately deferred, not urgent: converting `tests_smoke.py`'s 456 sequential checks to a real pytest suite (many checks share state built up by earlier ones in the same run — decoupling that is real work, not a mechanical conversion), and decomposing `GUIEngine._tick()` (785 lines, still one method in `gui/core.py`, "inherently cross-panel by design")
