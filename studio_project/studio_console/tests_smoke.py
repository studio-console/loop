"""Studio Console headless smoke test — extracted from studio_project.py's
STUDIO_HEADLESS block. Exercises ~456 checks across the FX/cue/fader/patch/
GUI-build pipeline with no real GUI or hardware (paired with
STUDIO_DRY_RUN). Moved as-is: converting this to a pytest-style suite is a
genuine possible improvement but explicitly out of scope for this split —
this file is the oracle every other phase was verified against, so it
shouldn't itself be in motion while everything else changes.

run_smoke_test(gui) takes the already-constructed GUIEngine instance as a
parameter (the one thing here with nowhere else to import it from — it's
built directly in studio_project.py, not any extracted module) and calls
sys.exit() itself at the end, exactly matching the original inline
behavior — the entry point calling this never expects it to return.
"""
...
# GUIEngine hasn't been extracted as its own importable module (it's
# composed directly in studio_project.py from the gui/ mixins) — not
# needed here anyway, since the caller passes the already-built
# instance in as a parameter.
#
# EXCEPT for testing GUIEngine's own static methods directly
# (_fpg_exec_for_slot/_fpg_slot_for_exec) below — GUIEngine is defined
# directly in studio_project.py (not extracted to its own module) at a
# position before this module gets lazily imported (only inside the
# `if STUDIO_HEADLESS:` branch, after `gui = GUIEngine(...)` has already
# run), so a module-level reach-back is safe.
from __main__ import GUIEngine


def run_smoke_test(gui):
