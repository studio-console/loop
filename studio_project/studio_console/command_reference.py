...
COMMAND_REFERENCE = [
    ("selection", [
        ...
        ("NEXT",                  "select the next fixture after the current selection (wraps) — steps sub-fixtures instead when only one sub is selected. Also bound to the → key"),
        ("PREV",                  "select the previous fixture before the current selection (wraps) — steps sub-fixtures instead when only one sub is selected. Also bound to the ← key"),
    ...
    ("colour & dim", [
        ...
        ("COL 3  /  COLOR 3",     "apply colour preset to selection — must be its own command, sent AFTER a selection command (e.g. '1.1' then 'COLOR 3'); 'COLOR' combined with a selection in the same line (e.g. '1.1 COLOR 3') is parsed as setting a moving-light COLOR-wheel channel instead, not a preset recall"),
        ("DIM PRESET 2",          "apply dim preset to selection — same rule as COLOR above: send the selection first, then 'DIM PRESET 2' as its own command"),
    ...
    ("record (saving presets/pools)", [
        ...
        ("LIST REFS GROUP 90",    "show every cue built from group 90 (whole-fixture members only)"),
        ...
        ("UPDATE GROUP 90",       "resync every cue built from group 90 to its current membership (after GROUP 90 ADD/REMOVE) — doesn't touch the programmer, since a group is a set of fixtures, not a value"),
    ...
    ("stack go/back", [
        ...
        ("STACK 2",              "give stack 2 focus — 'focus' means whichever fader/stack commands like RECORD CUE and UPDATE CUE target when you don't name one explicitly. Same as clicking the stack's name in the left column, or its id badge in the fader page grid"),
        ...
        ("stk 1 TRACKING ON",        "stk 1 (default): a cue fires with every earlier cue's values it doesn't override itself — GO, BACK, and GOTO all resolve the same tracked state, so BACK past a cue that set something explicitly correctly reverts, not just whatever's currently live"),
        ("stk 1 TRACKING OFF",       "stk 1: a cue fires with ONLY what's explicitly recorded into it — for a show deliberately recorded assuming no tracking"),
    ...
    ("faders & pages", [
        ...
        ("click a fader's id badge (fdrs grid)", "give that fader focus (same as STACK n) — white outline marks whichever one slot currently has focus"),
        ("running cue in fdrs grid", "the cue actually playing on a fader is highlighted blue in that fader's own cue list, independent of which fader has focus — several can be running and highlighted at once"),
    ]),
    ("attribute pools", [
    ...
    ("programmer", [
        ...
        ("UNDO TO 3",             "jump back multiple steps at once, to history entry 3 — same list shown by right-clicking the UNDO button"),
        ("right-click UNDO button", "opens the undo history popup — pick any earlier point to jump straight back to it, instead of stepping UNDO one at a time"),
    ...
    ("patch commands", [
        ...
        ("VIZ LAYOUT",             "list every fixture with a custom stage-view pixel grid (none listed = all fixtures auto-fit)"),
        ("VIZ LAYOUT 1 THRU 4 GRID 6x9 COLMAJOR", "give fixtures 1-4 a fixed 6-across x 9-down grid in the stage view, addressed down each column before moving to the next — for real wiring like several vertical strips side by side, which the default auto-fit arranges wrong (it always fills row by row, assuming a free choice of shape)"),
        ("VIZ LAYOUT 5 GRID 8x4",  "same, but row-major (left-to-right, top-to-bottom) — the default fill order when ROWMAJOR/COLMAJOR is left off"),
        ...
        ("VIZ POSITION 5 AT 3,0.5,-2 90", "same, plus a 90-degree yaw (spin left/right around the vertical axis)"),
        ("VIZ POSITION 5 AT 3,0.5,-2 90 -30", "same, plus a -30-degree pitch (nose tilted up 30 degrees — how the fixture is physically mounted, e.g. a truss par angled down at the stage would use a positive pitch instead)"),
        ("VIZ POSITION 1 THRU 4 CLEAR", "remove fixtures 1-4's custom placement, back to auto-arrange (a line/grid ordered by patch id)"),
        ("3D viz coordinate system", "units are FEET — one ground-grid square is 1x1ft, and each fixture's marker cube is exactly 1x1x1ft, so distances read directly off the grid. y is up — y=0 is the ground plane the grid is drawn on, a fixture sitting ON the floor has y=0.5 (its cube resting on the floor, not its center in the floor) — a truss-mounted fixture would use a larger y. x is left/right, z is depth (toward/away from the camera). yaw is degrees, 0 = facing +z, spinning left/right around the vertical (y) axis. pitch is degrees, 0 = level, tilting the fixture's own nose up/down (positive = down) around its own local axis — a placement property (how it's mounted), separate from a mover's live pan/tilt DMX output, which composes with it rather than replacing it. Leaving yaw or pitch off a VIZ POSITION AT resets yaw to 0 but leaves pitch as whatever it already was, so re-aiming a fixture's yaw doesn't undo its mount angle. Auto-arrange (no VIZ POSITION set) lines fixtures up along x at y=0.5 (resting on the floor), 2.5ft apart centre-to-centre, centered on x=0, ordered by fixture id, yaw=0, pitch=0 — a reasonable starting layout to build a request like 'arrange in a circle' or 'put the movers upstage, 6ft back' from, since fixtures.viz_position in the console state shows what's currently set (None = still auto-arranged) rather than requiring a guess."),
    ]),
    ("macros", [
    ...
    ("keyboard", [
        ...
        ("←  /  →",               "PREV / NEXT fixture (or sub-fixture) selection — same as typing the command, disabled while typing in the command line"),
        ...
        ("3d viz — view buttons", "iso/top/front/side — iso is the perspective view with the orbiting camera; top/front/side are flat orthographic plan/elevation views (no perspective, no camera to orbit) for placing something exactly when iso's foreshortening makes it hard to judge — top shows x/z, front shows x/y, side shows z/y"),
        ("3d viz — move a fixture", "left-click-drag it along the two axes the CURRENT view shows (x/z in iso and top, x/y in front, z/y in side); Shift-drag adjusts height (y) instead, in any view"),
        ("3d viz — rotate a fixture", "Ctrl+right-drag it to rotate yaw (spin left/right); Alt+right-drag to rotate pitch (nose up/down — how it's physically mounted, e.g. a truss par angled down at the stage) — works the same in every view"),
        ("3d viz — camera",       "in iso, right-drag empty space to orbit the camera (horizontal) and tilt it (vertical); the 'pause orbit' button stops/starts the automatic slow auto-orbit without affecting manual dragging (top/front/side have no camera to orbit)"),
        ("3d viz — fixture list", "a table below the canvas lists every fixture with editable x/y/z/yaw/pitch fields — type a number and press Enter for exact placement when even an ortho view's drag isn't precise enough; updates live as you drag in the canvas, and vice versa"),
    ]),
    ("status bar & quick controls", [
        ...
        ("sel: label",             "shows every currently-selected fixture/sub-fixture; hover it for the full list if it's truncated"),
        ("command log",            "scrolls and auto-follows the bottom; select and copy text out of it like any text field"),
    ]),
