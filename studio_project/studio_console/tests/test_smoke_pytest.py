"""pytest port of tests_smoke.py's ~456 checks.

tests_smoke.py (run via `STUDIO_DRY_RUN=1 STUDIO_HEADLESS=1 python3
studio_project.py`) remains the canonical oracle every phase of the
studio_console/ package split was verified against -- it is untouched by
this file and stays the source of truth. This is an additive, parallel
entry point: the exact same check sequence, generated mechanically by
converting every `_check(label, cond)` call into
`with subtests.test(msg=label): assert (cond)` (verified via AST
structural comparison against the original at generation time, not just
eyeballed), so pytest can report each of the ~456 checks individually
(pass/fail, -k filtering, JUnit XML for CI) instead of one hand-rolled
PASS/FAIL list printed to stdout.

Sequential, not independent: like the original, every check after the
first depends on state built up by the ones before it (the same
`run_command()` calls mutating the same `fader_pool`/`patch`/`stack_pool`
singletons) -- that interdependency is exactly why this is one long
`test_smoke_full_pipeline` function using pytest-subtests for per-check
visibility, not ~456 independent `test_*` functions. Splitting further
would mean re-deriving which locals cross which boundaries (the same
class of work done for GUIEngine._tick()'s decomposition) -- worth doing
later if finer-grained *isolation* (not just reporting) is ever needed,
but out of scope for this pass.

`gui` and every engine singleton (patch, fader_pool, stack_pool,
output_state, run_command, ...) come from actually running
studio_project.py itself via runpy, exactly as `python3 studio_project.py`
would build them -- see STUDIO_PYTEST_COLLECT in studio_project.py. This
guarantees the exact same construction order/wiring as a real launch,
with zero duplicated setup logic to drift out of sync.

Run with: pytest studio_console/tests/test_smoke_pytest.py -v
"""
...
_ENTRY = os.path.join(_REPO_ROOT, "studio_project.py")

os.environ.setdefault("STUDIO_DRY_RUN", "1")
os.environ.setdefault("STUDIO_HEADLESS", "1")
os.environ["STUDIO_PYTEST_COLLECT"] = "1"

_ns = runpy.run_path(_ENTRY, run_name="__main__")

# Pin sys.modules['__main__'] to the harvested namespace for the rest of
# this process. runpy restores the pre-call sys.modules['__main__'] once
# run_path() returns (so it doesn't permanently hijack the caller's real
# __main__) -- but studio_console/ leans on `from __main__ import X`
# deferred imports throughout (run_command, GUIEngine, _prog_time,
# save_show, network, Fader, ...), most of them function-local and
# triggered *later*, while the checks below are running (run_command's
# own recursive-dispatch branches in particular fire on nearly every
# command). Without this, those would ImportError the first time a
# check exercises one of those code paths, well after the harvest call
...
def test_smoke_full_pipeline(subtests):
    ...
    try:
        ...
        with subtests.test(msg="FX command applied to programmer"):
            assert ("FX" in r1)

        r2 = run_command("RECORD stk 1 CUE 5")
        with subtests.test(msg="cue recorded"):
            assert ("recorded" in r2 or "cue" in r2)

        run_command("GO stk 1 CUE 5")
        time.sleep(0.25)   # let FadeEngine/FXEngine tick at least once
        ex = fader_pool.get(1)
        with subtests.test(msg="fader has active FX after GO"):
            assert (len(ex._fx_ids) > 0)

        dmx = output_state.get_dmx_for_universe(1)
        with subtests.test(msg="DMX output computes without exception"):
            assert (len(dmx) == 512)

        # GO FADE — one-shot fade override
        _pt_before = dict(_prog_time)
        r_gf = run_command("GO FADE 7.5")
        with subtests.test(msg="GO FADE fires without error"):
            assert ("Cue" in r_gf or "GO" in r_gf)
        with subtests.test(msg="GO FADE restores prog_time.on after fire"):
            assert (_prog_time['on'] == _pt_before['on'])
        with subtests.test(msg="GO FADE restores prog_time.fade after fire"):
            assert (_prog_time['fade'] == _pt_before['fade'])

        # FADER SWAP
        _cs1_before = fader_pool.get(1).stack
        _cs2_before = fader_pool.get(2).stack
        r_swap = run_command("FADER SWAP 1 2")
        with subtests.test(msg="FADER SWAP swaps stk onto fader 1"):
            assert (fader_pool.get(1).stack is _cs2_before)
        with subtests.test(msg="FADER SWAP swaps stk onto fader 2"):
            assert (fader_pool.get(2).stack is _cs1_before)
        ...
        if _cf_f1 and _cf_f2:
            ...
            with subtests.test(msg="COPY FIXTURE copies sub channel to destination"):
                assert (_cf_red_src is not None and _cf_red_dst == _cf_red_src)
            with subtests.test(msg="COPY FIXTURE returns confirmation message"):
                assert ("copied fixture" in r_cf)
        r_cf_bad = run_command("COPY FIXTURE 999 TO 2")
        with subtests.test(msg="COPY FIXTURE rejects unknown source"):
            assert ("not patched" in r_cf_bad or "999" in r_cf_bad)
        ...
        if _fs_f1s and _fs_f2s:
            ...
            with subtests.test(msg="FIXTURE SWAP moves fixture 2 value to fixture 1"):
                assert (_sw_r1 == 200)
            with subtests.test(msg="FIXTURE SWAP moves fixture 1 value to fixture 2"):
                assert (_sw_r2 == 80)
            with subtests.test(msg="FIXTURE SWAP returns confirmation"):
                assert ("swapped" in r_swap.lower())
            prog.undo()
            _sw_u1 = prog.data.get(_fs_f1s, {}).get('red')
            _sw_u2 = prog.data.get(_fs_f2s, {}).get('red')
            with subtests.test(msg="FIXTURE SWAP pushes an undo snapshot (UNDO restores fixture 1)"):
                assert (_sw_u1 == 80)
            with subtests.test(msg="FIXTURE SWAP pushes an undo snapshot (UNDO restores fixture 2)"):
                assert (_sw_u2 == 200)
        prog.clear_programmer()
        ...
        if _fi:
            ...
            with subtests.test(msg="FIXTURE INFO shows fixture name"):
                assert (_fi.name in r_fi)
            with subtests.test(msg="FIXTURE INFO shows profile name"):
                assert (_fi.profile.name in r_fi)
            with subtests.test(msg="FIXTURE INFO shows channel list"):
                assert (any(ch in r_fi for ch in _fi.profile.channels))
        r_fi_bad = run_command("FIXTURE INFO 999")
        with subtests.test(msg="FIXTURE INFO rejects unknown fixture"):
            assert ("not patched" in r_fi_bad or "999" in r_fi_bad)
        ...
        with subtests.test(msg="PROGRAMMER STATS shows sub-fixture count"):
            assert ("sub-fixtures" in r_ps)
        with subtests.test(msg="PROGRAMMER STATS shows total params > 0"):
            assert ("total params" in r_ps and "total params    : 0" not in r_ps)
        prog.clear_programmer()
        r_ps_empty = run_command("PROGRAMMER STATS")
        with subtests.test(msg="PROGRAMMER STATS shows 0 params when clear"):
            assert ("total params    : 0" in r_ps_empty or "0" in r_ps_empty)

        # PROGRAMMER CAPTURE
        prog.clear_programmer()
        run_command("1 THRU 3")           # select fixtures 1-3
        r_cap = run_command("PROGRAMMER CAPTURE")
        with subtests.test(msg="PROGRAMMER CAPTURE returns confirmation"):
            assert ("captured" in r_cap)
        prog.clear_programmer()
        ...
        with subtests.test(msg="PROGRAMMER SAVE returns confirmation"):
            assert ("saved" in r_psnap.lower())
        with subtests.test(msg="PROGRAMMER SAVE stores snapshot"):
            assert (5 in _prog_snapshots)
        prog.clear_programmer()           # wipe programmer
        with subtests.test(msg="PROGRAMMER CLEAR removes values"):
            assert (not prog.data.get("1.1"))
        r_pload = run_command("PROGRAMMER LOAD 5")
        with subtests.test(msg="PROGRAMMER LOAD restores values"):
            assert (prog.data.get("1.1", {}).get('red') == 150)
        with subtests.test(msg="PROGRAMMER LOAD returns confirmation"):
            assert ("loaded" in r_pload.lower())
        r_psnaps = run_command("PROGRAMMER SNAPSHOTS")
        with subtests.test(msg="PROGRAMMER SNAPSHOTS lists the saved slot"):
            assert ("TestSnap" in r_psnaps)
        prog.clear_programmer()

        # SET DEFAULT / DEFAULT
        _saved_defaults = dict(_fixture_defaults)
        r_sd_dim  = run_command("SET DEFAULT DIM 0")
        with subtests.test(msg="SET DEFAULT DIM returns confirmation"):
            assert ("dim" in r_sd_dim)
        with subtests.test(msg="SET DEFAULT DIM stores 0.0 in _fixture_defaults"):
            assert (_fixture_defaults.get('dim') == 0.0)
        r_sd_clr  = run_command("SET DEFAULT CLR 5600")
        with subtests.test(msg="SET DEFAULT CLR stores kelvin"):
            assert (_fixture_defaults.get('kelvin') == 5600)
        r_sd_red  = run_command("SET DEFAULT RED 200")
        with subtests.test(msg="SET DEFAULT RED stores value"):
            assert (_fixture_defaults.get('red') == 200)
        r_default = run_command("DEFAULT")
        with subtests.test(msg="DEFAULT shows dim"):
            assert ("dim"    in r_default)
        with subtests.test(msg="DEFAULT shows kelvin"):
            assert ("kelvin" in r_default)
        r_bad     = run_command("SET DEFAULT FOOBAR 50")
        with subtests.test(msg="SET DEFAULT rejects unknown param"):
            assert ("unknown" in r_bad)
        # restore
        ...
        with subtests.test(msg="PROGRAMMER SHOW lists active fixture name"):
            assert (_f1_master is not None and _f1_master.name in r_pshow)
        with subtests.test(msg="PROGRAMMER SHOW shows channel value"):
            assert ("200" in r_pshow or "red" in r_pshow.lower())
        prog.clear_programmer()
        r_pshow_empty = run_command("PROGRAMMER SHOW")
        with subtests.test(msg="PROGRAMMER SHOW shows (empty) when clear"):
            assert ("empty" in r_pshow_empty)
        ...
        with subtests.test(msg="trigger_mode set"):
            assert (fader_pool.get(1).trigger_mode == 'flash')

        run_command("FADER 1 flash on")
        time.sleep(0.05)
        with subtests.test(msg="fader active after flash on"):
            assert (fader_pool.get(1).is_active)

        run_command("FADER 1 flash off")
        with subtests.test(msg="fader inactive after flash off"):
            assert (not fader_pool.get(1).is_active)
        ...
        with subtests.test(msg="RECORD COLOR from programmer"):
            assert ("recorded" in r_col or "no RGB" in r_col)

        run_command("ALL AT DIM 80")
        r_dim = run_command("RECORD DIM 1 TestDim")
        with subtests.test(msg="RECORD DIM from programmer"):
            assert ("recorded" in r_dim or "no dimmer" in r_dim)

        # Explicit-value record
        r_col2 = run_command("RECORD COLOR 2 BlueTest 0 0 255")
        with subtests.test(msg="RECORD COLOR explicit RGB"):
            assert ("recorded" in r_col2)

        r_dim2 = run_command("RECORD DIM 2 Half 50%")
        with subtests.test(msg="RECORD DIM explicit level"):
            assert ("recorded" in r_dim2)

        # LIST COLOR/DIM — verify no AttributeError on pool iteration
        r_lc = run_command("LIST COLOR")
        with subtests.test(msg="LIST COLOR no exception"):
            assert ("color" in r_lc.lower())

        r_ld = run_command("LIST DIM")
        with subtests.test(msg="LIST DIM no exception"):
            assert ("dim" in r_ld)
        ...
        for _cmd, _kw in [
            ...
            _r = run_command(_cmd)
            with subtests.test(msg=f"{_cmd!r} routes correctly"):
                assert (_kw.lower() in _r.lower())

        # LIST CUES and LIST CUES stk <n>
        _lc_r = run_command("LIST CUES")
        with subtests.test(msg="LIST CUES returns cue list for active stack"):
            assert ("stk " in _lc_r.lower() or "not found" in _lc_r.lower())
        _lc_cs1 = run_command("LIST CUES stk 1")
        with subtests.test(msg="LIST CUES stk 1 targets stack 1"):
            assert ("stk 1" in _lc_cs1.lower() or "not found" in _lc_cs1.lower())
        ...
        with subtests.test(msg="COPY COLOR routes to pool handler"):
            assert ("copied color" in r_cp_col)
        run_command("RECORD DIM 5 CopySrc 75%")
        r_cp_dim = run_command("COPY DIM 5 TO 6 CopiedDim")
        with subtests.test(msg="COPY DIM routes to pool handler"):
            assert ("copied dim" in r_cp_dim)
        ...
        with subtests.test(msg="CLEAR RATE deletes the preset"):
            assert ("cleared" in r_clr_rate.lower())
        with subtests.test(msg="CLEAR RATE actually removed it"):
            assert (rate_pool.get(9) is None)
        run_command("RECORD SIZEP 9 ClearMe 40")
        r_clr_size = run_command("CLEAR SIZEP 9")
        with subtests.test(msg="CLEAR SIZEP deletes the preset"):
            assert ("cleared" in r_clr_size.lower())
        run_command("RECORD SPREADP 9 ClearMe 40")
        r_clr_spread = run_command("CLEAR SPREADP 9")
        with subtests.test(msg="CLEAR SPREADP deletes the preset"):
            assert ("cleared" in r_clr_spread.lower())
        run_command('record form 9 ClearMe 0,0 1,1')
        r_clr_form = run_command("CLEAR FORM 9")
        with subtests.test(msg="CLEAR FORM deletes a custom form"):
            assert ("cleared" in r_clr_form.lower())
        r_clr_form_builtin = run_command("CLEAR FORM 1")
        with subtests.test(msg="CLEAR FORM protects built-in slot 1"):
            assert ("built-in" in r_clr_form_builtin.lower())
        ...
        with subtests.test(msg="COPY FORM routes to pool handler"):
            assert ("copied form" in r_cp_form)
        with subtests.test(msg="COPY FORM created the destination"):
            assert (form_pool.get(9) is not None)
        r_cp_form_builtin = run_command("COPY FORM 8 TO 2 Overwrite")
        with subtests.test(msg="COPY FORM protects built-in destination slots"):
        ...
        with subtests.test(msg="fader page cannot go below page 1"):
            assert (gui._fpg_page == 1)
        gui._fpg_step_page(1)
        gui._fpg_step_page(1)
        with subtests.test(msg="fader page increments normally"):
            assert (gui._fpg_page == 3)
        ...
        with subtests.test(msg="TAP computes BPM from two taps"):
            assert ("BPM" in _r_tap1 or "→" in _r_tap1)

        # MIDI text commands — add, list, remove
        _r_midi_map = run_command("MIDI CC 15 100 GO")
        with subtests.test(msg="MIDI CC maps correctly"):
            assert ("mapped" in _r_midi_map)
        _r_midi_list = run_command("LIST MIDI")
        with subtests.test(msg="LIST MIDI shows new mapping"):
            assert ("ch15" in _r_midi_list)
        _r_midi_rm = run_command("MIDI REMOVE CC 15 100")
        with subtests.test(msg="MIDI REMOVE CC removes mapping"):
            assert ("removed" in _r_midi_rm)
        _r_targets = run_command("MIDI TARGETS")
        with subtests.test(msg="MIDI TARGETS lists targets"):
            assert ("GO" in _r_targets)
        ...
        with subtests.test(msg="BLACKOUT overrides HIGHLIGHT in DMX output"):
            assert (max(_dmx_bbo) == 0)
        ...
        with subtests.test(msg="FREEZE snapshot holds the look"):
            assert (max(_dmx_frozen) > 0)
        run_command("BLACKOUT")
        _dmx_frozen_bbo = output_state.get_dmx_for_universe(1)
        with subtests.test(msg="BLACKOUT overrides FREEZE in DMX output"):
            assert (max(_dmx_frozen_bbo) == 0)
        run_command("BLACKOUT OFF")
        _dmx_frozen_restored = output_state.get_dmx_for_universe(1)
        with subtests.test(msg="FREEZE look restored after BLACKOUT OFF"):
            assert (_dmx_frozen_restored == _dmx_frozen)
        run_command("DMX 1 42")
        _dmx_frozen_override = output_state.get_dmx_for_universe(1)
        with subtests.test(msg="direct DMX override still applies during FREEZE"):
            assert (_dmx_frozen_override[0] == 42)
        run_command("CLEAR DMX")

        # GRANDMASTER
        output_state.master_level = 1.0
        r_gm_show = run_command("GRANDMASTER")
        with subtests.test(msg="GRANDMASTER (no args) shows current level"):
            assert ("%" in r_gm_show)
        r_gm_set = run_command("GM 75")
        with subtests.test(msg="GM 75 sets master to 75%"):
            assert (abs(output_state.master_level - 0.75) < 0.01)
        with subtests.test(msg="GM 75 returns confirmation with new level"):
            assert ("75" in r_gm_set)
        run_command("GM FULL")
        with subtests.test(msg="GM FULL sets master to 100%"):
            assert (output_state.master_level == 1.0)
        run_command("GM OUT")
        with subtests.test(msg="GM OUT sets master to 0%"):
            assert (output_state.master_level == 0.0)
        output_state.master_level = 1.0   # restore

        # SHOW INFO
        r_si = run_command("SHOW INFO")
        with subtests.test(msg="SHOW INFO returns multi-line overview"):
            assert (len(r_si.splitlines()) >= 5)
        with subtests.test(msg="SHOW INFO shows fixture count"):
            assert ("fixtures" in r_si)
        with subtests.test(msg="SHOW INFO shows master level"):
            assert ("master" in r_si)
        ...
        with subtests.test(msg="OUTPUT STATUS returns non-empty string"):
            assert (len(r_os) > 10)
        with subtests.test(msg="OUTPUT STATUS shows master level"):
            assert ("master=" in r_os.lower() or "Output" in r_os)
        prog.clear_programmer()
        ...
        with subtests.test(msg="SOLO still zeros non-solo fixtures during FREEZE"):
            assert (_dmx_frozen_solo[_other_out['address'] - 1] == 0)
        with subtests.test(msg="SOLO still passes the solo'd fixture during FREEZE"):
            assert (_dmx_frozen_solo[_solo_out['address'] - 1] > 0)
        ...
        with subtests.test(msg="RECORD GROUP from selection"):
            assert ("recorded" in r_grp)
        r_grp_recall = run_command("GROUP 9")
        with subtests.test(msg="GROUP recall"):
        ...
        with subtests.test(msg="GROUP ADD increases member count by 1"):
            assert (len(_g9.members) == _g9_before + 1)
        with subtests.test(msg="GROUP ADD returns confirmation"):
            assert ("added" in r_gadd.lower())
        r_gadd_dup = run_command("GROUP 9 ADD 4")
        with subtests.test(msg="GROUP ADD rejects duplicate fixture"):
            assert ("already" in r_gadd_dup.lower())
        r_grem = run_command("GROUP 9 REMOVE 4")
        with subtests.test(msg="GROUP REMOVE decreases member count by 1"):
            assert (len(_g9.members) == _g9_before)
        with subtests.test(msg="GROUP REMOVE returns confirmation"):
            assert ("removed" in r_grem.lower())
        r_grem_miss = run_command("GROUP 9 REMOVE 4")
        with subtests.test(msg="GROUP REMOVE rejects missing fixture"):
            assert ("not in group" in r_grem_miss.lower())
        ...
        with subtests.test(msg="record form custom breakpoints"):
            assert ("recorded" in r_form)
        r_form_list = run_command("FORM LIST")
        with subtests.test(msg="FORM LIST shows recorded form"):
            assert ("smokewave" in r_form_list.lower())
        ...
        with subtests.test(msg="RECORD FX from programmer"):
            assert ("recorded" in r_fx_rec)
        run_command("FX CLEAR")
        r_fx_fire = run_command("FIRE FX 9")
        with subtests.test(msg="FIRE FX reapplies preset"):
            assert ("FX" in r_fx_fire)
        run_command("FX CLEAR")
        ...
        with subtests.test(msg="fx_pool save/load preserves speed_id"):
            assert (_reloaded_layer.get("speed_id") == 3)
        ...
        with subtests.test(msg="RECORD FX from programmer"):
            assert ("recorded" in r_fx_rec2)
        with subtests.test(msg="RECORD FX preserves speed_id"):
            assert (fx_pool.get(20).layers[0].get("speed_id") == 7)
        run_command("FX CLEAR")
        ...
        for _attr in ("POSITION", "GOBO", "ZOOM", "FOCUS", "BEAM", "CONTROL"):
            ...
            with subtests.test(msg=f"RECORD {_attr} handles no-data case cleanly"):
                assert ("no" in r_attr.lower() and "data in programmer" in r_attr.lower())
        ...
        with subtests.test(msg="OSC /gma3/fader/1/1 reaches _osc_fader and sets grandmaster dim"):
            assert (abs(_fader_dim[0] - 0.42) < 1e-6)
        ...
        with subtests.test(msg="OSC /gma3/fader/1/3 sets fader 3's own level (not grandmaster)"):
            assert (abs(_osc_ex.level - 0.65) < 1e-6)
        ...
        with subtests.test(msg="OSC /gma3/key/1/3/go GOes fader 3 (routes through FADER 3 GO)"):
            assert (_osc_ex.is_active)
        ...
        with subtests.test(msg="OSC /gma3/key/1/3/flash press fires FADER 3 flash on"):
            assert (_osc_ex.is_active)
        ...
        with subtests.test(msg="OSC /gma3/key/1/3/flash release fires FADER 3 flash off"):
            assert (not _osc_ex.is_active)
        ...
        # AudioEngine.list_devices()/.start() read _AUDIO_AVAILABLE from
        # drivers/audio.py's own module namespace, so the mutation must go
        # through the qualified module reference, not a bare name — doubly
        # true now that this whole block lives inside run_smoke_test(): a
        # bare `_AUDIO_AVAILABLE = False` here would make Python treat
        # _AUDIO_AVAILABLE as a local for this entire function (standard
        # Python scoping — any assignment anywhere in a function makes the
        # name local throughout it), and the read on the old "save current
        # value" line above would then hit UnboundLocalError before that
        # assignment ever ran. This isn't a new bug from the split — the
        # original bare `_AUDIO_AVAILABLE = False` at module level in
        # studio_project.py was already redundant with the qualified-module
        # write by the time Phase 9 moved run_command's own check to
        # commands/io.py (which reads _audio_driver._AUDIO_AVAILABLE, not a
        # bare name) — this just removes bookkeeping nothing reads anymore.
        _prev_audio_avail = _audio_driver._AUDIO_AVAILABLE
        ...
        try:
            try:
                ...
                _list_ok = False
            with subtests.test(msg="AudioEngine.list_devices() doesn't raise when unavailable"):
                assert (_list_ok)

            try:
                _audio_probe.start()
                with subtests.test(msg="AudioEngine.start() raises RuntimeError when unavailable"):
                    assert (False)
            except RuntimeError:
                with subtests.test(msg="AudioEngine.start() raises RuntimeError when unavailable"):
                    assert (True)
            except Exception as _ae:
                with subtests.test(msg=f"AudioEngine.start() raised wrong exception "
                       f"({type(_ae).__name__}) when unavailable"):
                    assert (False)
            ...
            with subtests.test(msg="AUDIO OFF works pre-start"):
                assert (not audio_mapper.enabled)
            run_command("AUDIO ON")
            with subtests.test(msg="AUDIO ON enables mapper"):
                assert (audio_mapper.enabled)
            r_audio_status = run_command("AUDIO STATUS")
            with subtests.test(msg="AUDIO STATUS reports mapping ON"):
                assert ("mapping ON" in r_audio_status)
            run_command("AUDIO OFF")
            with subtests.test(msg="AUDIO OFF disables mapper"):
                assert (not audio_mapper.enabled)
            _prev_gain = audio_engine.gain
            run_command("AUDIO GAIN 5")
            with subtests.test(msg="AUDIO GAIN sets engine gain"):
                assert (audio_engine.gain == 5.0)
            audio_engine.gain = _prev_gain

            r_devices = run_command("AUDIO DEVICES")
            with subtests.test(msg="AUDIO DEVICES reports unavailable cleanly"):
                assert ("unavailable" in r_devices.lower())
            r_start = run_command("AUDIO START")
            with subtests.test(msg="AUDIO START reports failure cleanly (no crash)"):
                assert ("failed" in r_start.lower())
        finally:
        ...
        with subtests.test(msg="CLEAR stage 3 does not touch master level"):
            assert (output_state.master_level == 0.8)
        with subtests.test(msg="CLEAR stage 3 kills live fades"):
            assert (len(prog.live_fades) == 0)
        with subtests.test(msg="CLEAR stage 3 returns programmer output cleared"):
            assert ("output cleared" in _r_clear3)
        ...
        with subtests.test(msg="RECORD CUE stores FOLLOW time"):
            assert (_cue_99 is not None and
        ...
        with subtests.test(msg="COPY CUE copies follow_time"):
            assert (_cue_98 is not None and
               abs(getattr(_cue_98, 'follow_time', 0) - 3.5) < 0.01)
        with subtests.test(msg="COPY CUE copies note"):
            assert (_cue_98 is not None and
               getattr(_cue_98, 'note', '') == "test note")

        # MOVE CUE renumbers and removes source
        run_command("MOVE CUE 98 TO 97 stk 1")
        with subtests.test(msg="MOVE CUE creates destination"):
            assert (_cs_1.cues.get(97.0) is not None)
        with subtests.test(msg="MOVE CUE removes source"):
            assert (_cs_1.cues.get(98.0) is None)

        # GOTO non-existent cue returns error, not false success
        _r_goto_bad = run_command("GOTO 9999")
        with subtests.test(msg="GOTO non-existent cue returns error"):
            assert ("not found" in (_r_goto_bad or "").lower())
        ...
        if _gtfl_ex and _gtfl_ex.stack and _gtfl_ex.stack.cues:
            ...
            with subtests.test(msg="FADER GOTO FIRST positions stack at first cue"):
                assert (_gtfl_cs.current == _gtfl_first)
            run_command("FADER 1 GOTO LAST")
            with subtests.test(msg="FADER GOTO LAST positions stack at last cue"):
                assert (_gtfl_cs.current == _gtfl_last)

        # delete cue cleans up cue_pool
        run_command("ALL AT R 128 G 0 B 0")
        run_command("RECORD stk 1 CUE 96")
        with subtests.test(msg="delete cue: cue_pool stale ref cleaned"):
            assert (True)  # record stores in pool
        _pool_has_96_before = cue_pool.get(96) is not None
        run_command("delete cue 96 stk 1")
        with subtests.test(msg="delete cue removes from cue_pool"):
            assert (_pool_has_96_before and cue_pool.get(96) is None)

        # speed master: set/get BPM
        _r_spd = run_command("SPEED 4 200")
        with subtests.test(msg="SPEED command sets BPM"):
        ...
        with subtests.test(msg="FX inline BPM default before speed ref"):
            assert (True)
        ...
        with subtests.test(msg="FX layer rate_bpm uses speed master"):
            assert (_layer0 is None or abs(_layer0.rate_bpm - 333.0) < 0.1)
        ...
        with subtests.test(msg="waveform sine range [0,1]"):
            assert (_wv_range('sine',     Waveform.sine))
        with subtests.test(msg="waveform ramp range [0,1]"):
            assert (_wv_range('ramp',     Waveform.ramp))
        with subtests.test(msg="waveform square range [0,1]"):
            assert (_wv_range('square',   Waveform.square))
        with subtests.test(msg="waveform pulse range [0,1]"):
            assert (_wv_range('pulse',    Waveform.pulse))
        with subtests.test(msg="waveform triangle range [0,1]"):
            assert (_wv_range('triangle', Waveform.triangle))
        with subtests.test(msg="waveform sawtooth range [0,1]"):
            assert (_wv_range('sawtooth', Waveform.sawtooth))
        with subtests.test(msg="waveform flicker range [0,1]"):
            assert (all(0.0 <= Waveform.flicker(t/200.0, i) <= 1.0
        ...
        with subtests.test(msg="flicker has per-pixel variation"):
            assert (len(set(_fl_vals)) > 1)

        # Flicker time resolution: enough unique states per cycle for 44Hz
        _fl_cycle = [Waveform.flicker(t / 100.0, 0) for t in range(100)]
        with subtests.test(msg="flicker has ≥44 unique states/cycle"):
            assert (len(set(_fl_cycle)) >= 44)

        # Sine shape: trough at 0, peak at 0.5, back to trough at 1.0
        with subtests.test(msg="sine shape: trough at 0.0"):
            assert (abs(Waveform.sine(0.0) - 0.0) < 0.01)
        with subtests.test(msg="sine shape: peak at 0.5"):
            assert (abs(Waveform.sine(0.5) - 1.0) < 0.01)

        # Pulse duty cycle: on for exactly 25% of a cycle
        _pulse_on = sum(1 for t in range(1000) if Waveform.pulse(t/1000.0) > 0.5)
        with subtests.test(msg="pulse duty cycle is 25%"):
            assert (abs(_pulse_on - 250) <= 2)

        # Strobe shorthand: STROBE creates a pulse dim FX layer
        run_command("FX CLEAR")
        _r_strobe = run_command("STROBE 120")
        with subtests.test(msg="STROBE creates FX layer"):
            assert (active_fx != [] or "FX" in (_r_strobe or ""))
        ...
        with subtests.test(msg="STROBE CLEAR removes dim FX"):
            assert (not _strobe_still)
        ...
        with subtests.test(msg="RAINBOW creates red layer"):
            assert ('red'   in _rainbow_chans)
        with subtests.test(msg="RAINBOW creates green layer"):
            assert ('green' in _rainbow_chans)
        with subtests.test(msg="RAINBOW creates blue layer"):
            assert ('blue'  in _rainbow_chans)
        ...
        if len(_rb_layers) >= 3:
            ...
            with subtests.test(msg="RAINBOW phases spaced ~0.33 apart"):
                assert (abs(_rb_ph[1] - _rb_ph[0] - 0.333) < 0.01 or
                   abs(_rb_ph[2] - _rb_ph[1] - 0.333) < 0.01)
        else:
            with subtests.test(msg="RAINBOW phases (need 3 layers)"):
                assert (False)
        ...
        if _sp_layer and len(_sp_layer._offsets) >= 2:
            with subtests.test(msg="spread=100 creates non-zero offsets"):
                assert (len(set(round(o, 4) for o in _sp_layer._offsets)) > 1)
        else:
            with subtests.test(msg="spread=100 (need ≥2 targets)"):
                assert (_sp_layer is None)
        ...
        if _sz_layer:
            ...
            with subtests.test(msg="FX size=50 gives max ~127 DMX"):
                assert (_sz_max <= 128.0)
        else:
            with subtests.test(msg="FX size=50 (layer needed)"):
                assert (False)
        ...
        with subtests.test(msg="dim FX layer channel is 'dim'"):
            assert (_dm_layer is not None)
        ...
        with subtests.test(msg="bounce direction stored"):
            assert (_bn_layer is not None and _bn_layer.direction == 'bounce')
        ...
        if _bk_layer and len(_bk_layer._offsets) >= 6:
            ...
            with subtests.test(msg="block_size=3 groups adjacent targets (offsets equal)"):
                assert (_bk_off[0] == _bk_off[1] == _bk_off[2] and
                   _bk_off[3] == _bk_off[4] == _bk_off[5])
        else:
            with subtests.test(msg="block_size=3 (need ≥6 targets)"):
                assert (True)
        ...
        if _if_layer:
            ...
            with subtests.test(msg="infade envelope starts near 0"):
                assert (_if_layer._last_env < 0.5)
        else:
            with subtests.test(msg="infade (layer needed)"):
                assert (False)
        ...
        with subtests.test(msg="cue tracking: cue 1 sets dim=0.8"):
            assert (abs(_layer.get('_test_fid', {}).get('dim', -1) - 0.8) < 0.001)

        # cue 2 fires (no dim in data_to) — should track dim=0.8
        _layer = _sim_fade(_layer, {})
        with subtests.test(msg="cue tracking: cue 2 tracks dim from cue 1"):
            assert (abs(_layer.get('_test_fid', {}).get('dim', -1) - 0.8) < 0.001)

        # cue 3 fires (also empty) — should still track dim=0.8
        _layer = _sim_fade(_layer, {})
        with subtests.test(msg="cue tracking: cue 3 still tracks dim (no stale zero)"):
            assert (abs(_layer.get('_test_fid', {}).get('dim', -1) - 0.8) < 0.001)

        # fx_outfade field: cue class should have it, default None
        _fxo_cue = Cue(1.0, "fxout")
        with subtests.test(msg="Cue.fx_outfade defaults to None"):
            assert (_fxo_cue.fx_outfade is None)

        # FXOUTFADE keyword in timing edit
        _apply_timing_edit(_fxo_cue, "FXOUTFADE 2.5")
        with subtests.test(msg="FXOUTFADE sets cue.fx_outfade"):
            assert (_fxo_cue.fx_outfade == 2.5)
        ...
        with subtests.test(msg="COPY stk preserves cue.fx_outfade"):
            assert (_fxo_dst_cs is not None and _fxo_dst_cs.cues.get(1.0) is not None and
               _fxo_dst_cs.cues[1.0].fx_outfade == 3.25)

        run_command("COPY stk 91 CUE 1 TO stk 93 CUE 1")
        _fxo_dst_cs2 = stack_pool.get(93)
        with subtests.test(msg="COPY CUE (single cue) preserves cue.fx_outfade"):
            assert (_fxo_dst_cs2 is not None and _fxo_dst_cs2.cues.get(1.0) is not None and
               _fxo_dst_cs2.cues[1.0].fx_outfade == 3.25)

        run_command("MOVE stk 91 CUE 1 TO stk 94 CUE 1")
        _fxo_dst_cs3 = stack_pool.get(94)
        with subtests.test(msg="MOVE CUE preserves cue.fx_outfade"):
        ...
        run_command("FX CLEAR")
        with subtests.test(msg="FX CLEAR clears fader FX (fader._fx_ids empty)"):
            assert (not _ex0._fx_ids)
        ...
        with subtests.test(msg="FX CLEAR with selection clears only selected fixtures (programmer)"):
            assert (_cleared_sel)
        ...
        with subtests.test(msg="CLEAR COLOUR pre-check: red was set"):
            assert (_pre_red == 255)
        run_command("CLEAR COLOUR")
        _post_rgb = prog.data.get(_sub1, {}).get('red') if _sub1 else None
        _post_dim = prog.data.get("1", {}).get('dim')
        with subtests.test(msg="CLEAR COLOUR zeroes RGB and leaves dim intact"):
            assert (_post_rgb == 0 and _post_dim == _pre_dim)
        ...
        with subtests.test(msg="CLEAR DIM zeroes dim, leaves RGB intact"):
            assert (_post_dim2 == 0.0 and _post_red2 == 200)

        # stk n WRAP ON/OFF — clean restart at top after last cue
        run_command("RECORD STACK 99 WrapTest")
        _stk99 = stack_pool.get(99)
        with subtests.test(msg="stk WRAP: default is False"):
            assert (_stk99.wrap is False)
        run_command("stk 99 WRAP ON")
        with subtests.test(msg="stk 99 WRAP ON sets .wrap = True"):
            assert (_stk99.wrap is True)
        run_command("stk 99 WRAP OFF")
        with subtests.test(msg="stk 99 WRAP OFF sets .wrap = False"):
            assert (_stk99.wrap is False)
        ...
        with subtests.test(msg="stk INFO rejects unknown stack"):
            assert ("not found" in r_csi_bad)
        ...
        with subtests.test(msg="stk REVERSE reverses cue order"):
            assert (_rev_names == list(reversed(_orig_names)))
        with subtests.test(msg="stk REVERSE returns confirmation"):
            assert ("reversed" in r_rev)
        with subtests.test(msg="stk REVERSE resets current position to None"):
            assert (_stk94.current is None)
        ...
        with subtests.test(msg="stk EXTRACT creates new stack in target slot"):
            assert (_stk98 is not None)
        with subtests.test(msg="stk EXTRACT new stack has exactly one cue"):
            assert (_stk98 is not None and len(_stk98.cues) == 1)
        with subtests.test(msg="stk EXTRACT preserves cue name"):
            assert (_stk98 is not None and list(_stk98.cues.values())[0].name.lower() == "extcue2")
        with subtests.test(msg="stk EXTRACT returns 'Extracted' confirmation"):
            assert ("extracted" in r_ext)
        with subtests.test(msg="stk EXTRACT source stack unchanged (still 2 cues)"):
            assert (len(_stk97.cues) == 2)
        with subtests.test(msg="stk EXTRACT carries source note to new stack"):
            assert (_stk98 is not None and _stk98.note == "Dark Moody Show")
        with subtests.test(msg="stk EXTRACT carries source wrap flag to new stack"):
            assert (_stk98 is not None and _stk98.wrap is True)
        ...
        with subtests.test(msg="stk DUPLICATE creates new stack in target slot"):
            assert (_stk100 is not None)
        with subtests.test(msg="stk DUPLICATE copies all cues"):
            assert (_stk100 is not None and len(_stk100.cues) == len(_cs99dup.cues))
        with subtests.test(msg="stk DUPLICATE is a deep copy (modifying source doesn't affect copy)"):
            assert (_stk100 is not None and _stk100.cues is not _cs99dup.cues)
        with subtests.test(msg="stk DUPLICATE returns 'Duplicated' confirmation"):
            assert ("duplicated" in r_dup)
        with subtests.test(msg="stk DUPLICATE source is unchanged"):
            assert (len(_cs99dup.cues) == 2)
        with subtests.test(msg="stk DUPLICATE carries source note to new stack"):
            assert (_stk100 is not None and _stk100.note == "Act 2 Opener")
        with subtests.test(msg="stk DUPLICATE carries source wrap flag to new stack (regression check)"):
            assert (_stk100 is not None and _stk100.wrap is True)
        ...
        with subtests.test(msg="stk RENUMBER STEP 10 gives multiples of 10"):
            assert (_ren_nums == [10.0, 20.0, 30.0])
        with subtests.test(msg="stk RENUMBER STEP returns confirmation"):
            assert ("renumbered" in r_ren.lower())
        ...
        with subtests.test(msg="CUE SHIFT moves cue to new number (3→8)"):
            assert (3.0 not in _shift_nums and 8.0 in _shift_nums)
        with subtests.test(msg="CUE SHIFT does not disturb other cues"):
            assert (7.0 in _shift_nums)
        with subtests.test(msg="CUE SHIFT returns confirmation with new number"):
            assert ("8" in r_shift)
        ...
        with subtests.test(msg="STACK MERGE adds src cues to dst"):
            assert (_n_after == _n_before + len(_stk91.cues))
        with subtests.test(msg="STACK MERGE returns confirmation"):
            assert ("merged" in r_merge)
        # Src cue numbers in dst should be offset past dst's original last cue
        _merged_num = max(_stk92._sorted_cue_numbers())
        with subtests.test(msg="STACK MERGE renumbers merged cues after dst's last cue"):
            assert (_merged_num > 1.0)
        r_merge_bad = run_command("STACK MERGE 9999 INTO 92")
        with subtests.test(msg="STACK MERGE rejects unknown source"):
            assert ("not found" in r_merge_bad)
        ...
        with subtests.test(msg="stk BACK wrap-around (first->last) clears LTP-bleed layer when WRAP ON"):
            assert ('__bleed_sentinel__' not in _ex98.layer)

        _stk98.wrap    = False
        _stk98.current = _stk98._sorted_cue_numbers()[0]
        _ex98.layer['__bleed_sentinel2__'] = {'red': 99}
        _stk98.back(patch, fade_engine, _ex98)
        with subtests.test(msg="stk BACK wrap-around leaves layer intact when WRAP OFF"):
            assert ('__bleed_sentinel2__' in _ex98.layer)
        ...
        _stk98.back(patch, fade_engine, _ex98)
        with subtests.test(msg="stk BACK non-wrap step leaves layer intact even with WRAP ON"):
            assert ('__bleed_sentinel3__' in _ex98.layer)
        ...
        with subtests.test(msg="UNDO pre-check: post-undo-marker red was set"):
            assert (prog.data.get(_sub1, {}).get('red') == 250)
        run_command("UNDO")
        with subtests.test(msg="UNDO restores prior programmer values"):
            assert (prog.data.get(_sub1, {}).get('red') == 10)
        with subtests.test(msg="UNDO keeps output_state.programmer_layer aliased to prog.data "
               "(same object identity, not a stale copy)"):
            assert (output_state.programmer_layer is prog.data)
        run_command("1 THRU 3 R 77")
        with subtests.test(msg="UNDO: post-undo edits are visible through the aliased "
               "programmer_layer used by real DMX output"):
            assert (output_state.programmer_layer.get(_sub1, {}).get('red') == 77)
        ...
        with subtests.test(msg="FADER 1 BTN A FLASH sets btn_a to FLASH"):
            assert (_fpg_ex.btn_a == 'FLASH')
        r_btn2 = run_command("FADER 1 BTN A GO")
        with subtests.test(msg="FADER 1 BTN A GO restores btn_a to GO"):
            assert (_fpg_ex.btn_a == 'GO')
        r_btn3 = run_command("FADER 1 BTN")  # missing slot → usage hint
        with subtests.test(msg="FADER 1 BTN without slot returns current state"):
            assert ("btn" in r_btn3.lower() or "usage" in r_btn3.lower() or "A=" in r_btn3)

        # RATE+/RATE- and RATE RESET smoke tests
        _rate_ex = fader_pool.get(2)
        _rate_ex.rate_factor = 1.0
        run_command("FADER 2 RATE+")
        with subtests.test(msg="FADER 2 RATE+ increases rate_factor to ~1.25"):
            assert (abs(_rate_ex.rate_factor - 1.25) < 0.01)
        run_command("FADER 2 RATE-")
        with subtests.test(msg="FADER 2 RATE- returns rate_factor to ~1.00"):
            assert (abs(_rate_ex.rate_factor - 1.0) < 0.01)
        run_command("FADER 2 RATE+")
        run_command("FADER 2 RATE RESET")
        with subtests.test(msg="FADER 2 RATE RESET returns rate_factor to 1.0"):
            assert (_rate_ex.rate_factor == 1.0)
        run_command("FADER 2 RATE 3.0")
        with subtests.test(msg="FADER 2 RATE 3.0 sets rate_factor to 3.0"):
            assert (abs(_rate_ex.rate_factor - 3.0) < 0.01)
        run_command("FADER 2 RATE RESET")
        r_rate_btn = run_command("FADER 2 BTN C RATE+")
        with subtests.test(msg="FADER 2 BTN C RATE+ sets btn_c to RATE+"):
            assert (_rate_ex.btn_c == 'RATE+')
        run_command("FADER 2 BTN C STOP")  # restore

        # FADER LABEL
        _lbl_ex = fader_pool.get(1)
        r_lbl = run_command("FADER 1 LABEL Main Show")
        with subtests.test(msg="FADER LABEL sets label on fader"):
            assert (_lbl_ex.label == "Main Show")
        with subtests.test(msg="FADER LABEL returns confirmation"):
            assert ("Main Show" in r_lbl)
        r_lbl_list = run_command("LIST FADER")
        with subtests.test(msg="LIST FADER shows label"):
            assert ("Main Show" in r_lbl_list)
        run_command("FADER 1 LABEL")  # clear
        with subtests.test(msg="FADER 1 LABEL (no text) clears label"):
            assert (_lbl_ex.label == "")

        # FADER INFO
        r_fi1 = run_command("FADER 1 INFO")
        with subtests.test(msg="FADER INFO shows level"):
            assert ("Level" in r_fi1)
        with subtests.test(msg="FADER INFO shows buttons"):
            assert ("Buttons" in r_fi1)
        _fi1_ex = fader_pool.get(1)
        if _fi1_ex.stack:
            with subtests.test(msg="FADER INFO shows stack name"):
                assert (_fi1_ex.stack.name in r_fi1)
        r_fi1_stat = run_command("FADER 1 STATUS")
        with subtests.test(msg="FADER STATUS alias works"):
            assert ("Level" in r_fi1_stat)
        ...
        if _clr_cs:
            ...
            with subtests.test(msg="FADER CLEAR resets stack position to None"):
                assert (_clr_cs.current is None)
            with subtests.test(msg="FADER CLEAR returns confirmation"):
                assert ("cleared" in r_fc or "reset" in r_fc)

        # FADER ALL CLEAR
        if _clr_cs:
            _clr_cs.current = 2  # set a position to confirm it gets reset
        r_fac = run_command("FADER ALL CLEAR")
        with subtests.test(msg="FADER ALL CLEAR returns 'cleared' confirmation"):
            assert ("cleared" in r_fac)
        with subtests.test(msg="FADER ALL CLEAR resets position of stack in fader 1"):
            assert (_clr_cs is None or _clr_cs.current is None)

        # FADER LOOP ON/OFF
        if _clr_cs:
            r_loop_on  = run_command("FADER 1 LOOP ON")
            with subtests.test(msg="FADER LOOP ON enables wrap on assigned stack"):
                assert (_clr_cs.wrap is True)
            with subtests.test(msg="FADER LOOP ON returns confirmation"):
                assert ("loop" in r_loop_on.lower() or "wrap" in r_loop_on.lower())
            r_loop_off = run_command("FADER 1 LOOP OFF")
            with subtests.test(msg="FADER LOOP OFF disables wrap on assigned stack"):
                assert (_clr_cs.wrap is False)
        ...
        with subtests.test(msg="LOAD SHOW-style OSC reload restores saved targets"):
            assert ("smoketest_stale" in osc._clients)
        with subtests.test(msg="LOAD SHOW-style OSC reload drops targets not in the saved show"):
            assert ("smoketest_extra" not in osc._clients)
        ...
        with subtests.test(msg="LOAD SHOW-style FX reload restores saved fx_params"):
            assert (_fx_params['rate_bpm'] == 60.0)

        # BACKUP command
        r_bk = run_command("BACKUP")
        with subtests.test(msg="BACKUP creates a timestamped save"):
            assert ("backup_" in r_bk and "saved" in r_bk.lower())
        ...
        with subtests.test(msg="Generic_Moving profile registered"):
            assert (_ml_profile is not None)
        ...
        if _ml_profile:
            ...
            with subtests.test(msg="moving light patched"):
                assert (_ml_fix is not None)
            ...
            _ml_sub_fid = "50.1"
            with subtests.test(msg="programmer stores pan"):
                assert (prog.data.get(_ml_sub_fid, {}).get('pan') == 200)
            with subtests.test(msg="programmer stores tilt"):
                assert (prog.data.get(_ml_sub_fid, {}).get('tilt') == 64)
            with subtests.test(msg="programmer stores gobo"):
                assert (prog.data.get(_ml_sub_fid, {}).get('gobo') == 10)
            ...
            with subtests.test(msg="cue playback drives pan in DMX"):
                assert (_ml_dmx[_ml_base + _pan_off] == 200)
            with subtests.test(msg="cue playback drives tilt in DMX"):
                assert (_ml_dmx[_ml_base + _tilt_off] == 64)
            with subtests.test(msg="cue playback drives gobo in DMX"):
                assert (_ml_dmx[_ml_base + _gobo_off] == 10)
            ...
            with subtests.test(msg="programmer attr channels visible in DMX output"):
                assert (_ml_dmx2[_ml_base + _pan_off] == 127)

            # Position pool: record and apply
            r_pos = run_command("RECORD POSITION 1 SmokePos")
            with subtests.test(msg="RECORD POSITION from moving light programmer"):
                assert ("Recorded" in r_pos or "no position" not in r_pos.lower())
            prog.clear_programmer()
            run_command("50")
            run_command("POSITION 1")
            with subtests.test(msg="POSITION 1 restores pan to programmer"):
                assert (prog.data.get(_ml_sub_fid, {}).get('pan') is not None)

            # FX on attribute channel: FX SINE PAN should create a layer
            run_command("FX CLEAR")
            run_command("50")
            r_pan_fx = run_command("FX SINE PAN BPM 30 SIZE 50")
            with subtests.test(msg="FX SINE PAN accepted"):
                assert ("Applied FX" in r_pan_fx)
            _pan_fx_layer = next(
                (l for l in (active_fx or []) if l.channel == 'pan'), None)
            with subtests.test(msg="FX SINE PAN creates layer with channel=pan"):
                assert (_pan_fx_layer is not None)
            run_command("FX CLEAR")
            ...
            with subtests.test(msg="dimmer profile channel outputs master dim correctly"):
                assert (_ml_dmx3[_ml_base + _dim_off] == 255)
            ...
            with subtests.test(msg="SNAPSHOT creates cue with attr channel data"):
                assert (_snap_cue is not None and
        ...
        with subtests.test(msg="AT 50 sets dim to 50%"):
            assert (abs(prog.data.get('1', {}).get('dim', 0) - 0.5) < 0.01)
        run_command("AT +20")                     # relative: 50 + 20 = 70%
        with subtests.test(msg="AT +20 increases dim by 20pp"):
            assert (abs(prog.data.get('1', {}).get('dim', 0) - 0.7) < 0.01)
        run_command("AT -10")                     # relative: 70 - 10 = 60%
        with subtests.test(msg="AT -10 decreases dim by 10pp"):
            assert (abs(prog.data.get('1', {}).get('dim', 0) - 0.6) < 0.01)
        # RGB relative
        run_command("AT R 100")
        run_command("AT R +50")                   # 100 + 50 = 150
        with subtests.test(msg="AT R +50 increases red by 50"):
            assert (prog.data.get('1.1', {}).get('red') == 150)
        run_command("AT R +200")                  # clamp at 255
        with subtests.test(msg="AT R +200 clamps to 255"):
            assert (prog.data.get('1.1', {}).get('red') == 255)
        prog.clear_programmer()
        ...
        with subtests.test(msg="HUE 0 sets red=255, green=0, blue=0"):
            assert (prog.data.get('1.1', {}).get('red') == 255 and
               prog.data.get('1.1', {}).get('green') == 0 and
               prog.data.get('1.1', {}).get('blue') == 0)
        run_command("AT HUE 120")     # pure green
        with subtests.test(msg="HUE 120 sets green=255"):
            assert (prog.data.get('1.1', {}).get('green') == 255 and
               prog.data.get('1.1', {}).get('red') == 0)
        run_command("AT HUE 240")     # pure blue
        with subtests.test(msg="HUE 240 sets blue=255"):
            assert (prog.data.get('1.1', {}).get('blue') == 255 and
               prog.data.get('1.1', {}).get('red') == 0)
        run_command("AT HUE 60 SAT 100 VAL 150")   # VAL over 100 must clamp
        with subtests.test(msg="HUE VAL clamps at 100 (no channel over 255)"):
            assert (prog.data.get('1.1', {}).get('red') == 255 and
               prog.data.get('1.1', {}).get('green') == 255)
        run_command("AT HUE 0 SAT -50")            # SAT under 0 must clamp to 0 (grey/white)
        with subtests.test(msg="HUE SAT clamps at 0 (grey output, not a negative-saturation artifact)"):
            assert (prog.data.get('1.1', {}).get('red') == 255 and
        ...
        with subtests.test(msg="CT 9000 (cool) sets blue=255"):
            assert (prog.data.get('1.1', {}).get('blue') == 255)
        with subtests.test(msg="CT 9000 (cool) sets red < 255"):
            assert ((prog.data.get('1.1', {}).get('red') or 0) < 255)
        prog.clear_programmer()
        run_command("1")
        run_command("AT CT 2700")   # warm tungsten — red = 255, blue ~ low
        with subtests.test(msg="CT 2700 (warm) sets red=255"):
            assert (prog.data.get('1.1', {}).get('red') == 255)
        with subtests.test(msg="CT 2700 (warm) sets blue < 200"):
            assert ((prog.data.get('1.1', {}).get('blue') or 0) < 200)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT FLIP R inverts red channel"):
            assert (prog.data.get('1.1', {}).get('red') == 195)
        run_command("AT R 0")
        run_command("AT FLIP R")         # 255 - 0 = 255
        with subtests.test(msg="AT FLIP R on 0 gives 255"):
            assert (prog.data.get('1.1', {}).get('red') == 255)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT RANDOM R sets red on all selected fixtures"):
            assert (all(v is not None for v in _rnd_reds))
        with subtests.test(msg="AT RANDOM R values are in valid range"):
            assert (all(0 <= v <= 255 for v in _rnd_reds if v is not None))
        ...
        with subtests.test(msg="AT RANDOM G MASTER applies same value to all subs of a master"):
            assert (len(set(v for v in _grn_subs if v is not None)) == 1)
        ...
        with subtests.test(msg="AT RANDOM DIM sets dim on all selected masters"):
            assert (all(v is not None for v in _rnd_dims))
        with subtests.test(msg="AT RANDOM DIM values are in valid range 0–1"):
            assert (all(0.0 <= v <= 1.0 for v in _rnd_dims if v is not None))
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT BRIGHTEST R stamps max value (200) to all subs"):
            assert (all(v == 200 for v in _bd_vals if v is not None))
        ...
        with subtests.test(msg="AT DARKEST R stamps min value (50) to all subs"):
            assert (all(v == 50 for v in _dk_vals if v is not None))
        ...
        with subtests.test(msg="AT AVERAGE R stamps mean value (100) to all subs"):
            assert (all(v == 100 for v in _avg_vals if v is not None))
        ...
        with subtests.test(msg="AT CLAMP R clamps below-lo value up to lo (50)"):
            assert (_cl_vals[0] == 50)
        with subtests.test(msg="AT CLAMP R leaves in-range value unchanged (100)"):
            assert (_cl_vals[1] == 100)
        with subtests.test(msg="AT CLAMP R clamps above-hi value down to hi (200)"):
            assert (_cl_vals[2] == 200)
        ...
        with subtests.test(msg="AT CLAMP DIM: 10% → 20% (clamped up to lo)"):
            assert (abs((_cd_dims[0] or 0) - 0.2) < 0.01)
        with subtests.test(msg="AT CLAMP DIM: 50% unchanged (in range)"):
            assert (abs((_cd_dims[1] or 0) - 0.5) < 0.01)
        with subtests.test(msg="AT CLAMP DIM: 90% → 80% (clamped down to hi)"):
            assert (abs((_cd_dims[2] or 0) - 0.8) < 0.01)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT BRIGHTEST DIM: all stamped to max (0.8)"):
            assert (all(abs((v or 0) - 0.8) < 0.01 for v in _bd_d))
        run_command("1 THRU 3 AT DARKEST DIM")
        _dk_d = [prog.data.get(str(i), {}).get('dim') for i in range(1, 4)]
        with subtests.test(msg="AT DARKEST DIM: all stamped to min (0.8 after BRIGHTEST)"):
            assert (all(abs((v or 0) - 0.8) < 0.01 for v in _dk_d))
        ...
        with subtests.test(msg="AT AVERAGE DIM: all stamped to mean (20+60+100)/3=60%"):
            assert (all(abs((v or 0) - 0.6) < 0.01 for v in _av_d))
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT INVERT DIM: 30% → 70%"):
            assert (all(abs((v or 0) - 0.7) < 0.01 for v in _inv_d))
        run_command("1 THRU 2 AT SCALE DIM 50")   # 70% × 50% = 35%
        _sc_d = [prog.data.get(str(i), {}).get('dim') for i in range(1, 3)]
        with subtests.test(msg="AT SCALE DIM 50: 70% × 50% = 35%"):
            assert (all(abs((v or 0) - 0.35) < 0.01 for v in _sc_d))
        run_command("1 THRU 2 AT SCALE DIM 200")  # 35% × 200% = 70% (≤ 1.0)
        _sc_d2 = [prog.data.get(str(i), {}).get('dim') for i in range(1, 3)]
        with subtests.test(msg="AT SCALE DIM 200: 35% × 200% = 70%"):
            assert (all(abs((v or 0) - 0.7) < 0.01 for v in _sc_d2))
        ...
        with subtests.test(msg="AT WOBBLE DIM: values remain in 0–1 range"):
            assert (all(0.0 <= (v or 0) <= 1.0 for v in _wb_d))
        with subtests.test(msg="AT WOBBLE DIM: values are near the seed (within 10%)"):
            assert (all(abs((v or 0) - 0.5) <= 0.10 for v in _wb_d))
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT STEP R fixture 1 unchanged (offset 0)"):
            assert (_st_vals[0] == 50)
        with subtests.test(msg="AT STEP R fixture 2 offset +20 (70)"):
            assert (_st_vals[1] == 70)
        with subtests.test(msg="AT STEP R fixture 3 offset +40 (90)"):
            assert (_st_vals[2] == 90)
        ...
        with subtests.test(msg="AT MIRROR R swaps first ↔ last (fixture 1 gets fixture 3's value)"):
            assert (_mir_r1 == 200)
        with subtests.test(msg="AT MIRROR R middle fixture gets its own mirror (fixture 3 gets fixture 1's value)"):
            assert (_mir_r3 == 10)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT INVERT R: 100 → 155 (255-100)"):
            assert (_inv_r1 == 155)
        with subtests.test(msg="AT INVERT R: 0 → 255 (255-0)"):
            assert (_inv_r2 == 255)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT SCALE R 50%: 200 → 100"):
            assert (_sc_r1 == 100)
        with subtests.test(msg="AT SCALE R 50%: 100 → 50"):
            assert (_sc_r2 == 50)
        run_command("1 AT R 200")
        run_command("1 AT SCALE R 200")   # 200% of 200 = 400 → clamped to 255
        _sc_clamp = prog.data.get("1.1", {}).get('red')
        with subtests.test(msg="AT SCALE R clamps at 255"):
            assert (_sc_clamp == 255)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT WOBBLE R keeps values in 0-255 range"):
            assert (all(v is not None and 0 <= v <= 255 for v in _wb_vals))
        with subtests.test(msg="AT WOBBLE R values are near the seed (within 50)"):
            assert (all(v is not None and abs(v - 128) <= 50 for v in _wb_vals))
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT CLEAR R removes red from programmer"):
            assert (_ac_r is None)
        with subtests.test(msg="AT CLEAR R leaves other channels intact"):
            assert (_ac_g == 100)
        run_command("1 AT CLEAR")         # remove all channels for fixture 1
        _ac_all = prog.data.get("1.1", {})
        with subtests.test(msg="AT CLEAR (no ch) removes all channels for selection"):
            assert (not _ac_all)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT NORMALIZE R: highest value becomes 255"):
            assert (_nrm_r[2] == 255)
        with subtests.test(msg="AT NORMALIZE R: proportional scale (50→64, 100→128)"):
            assert (_nrm_r[0] == 64 and _nrm_r[1] == 128)
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT COPY: dimmer copied to fixture 2"):
            assert (abs((_cp_dim2 or 0) - 0.8) < 0.01)
        with subtests.test(msg="AT COPY: dimmer copied to fixture 3"):
            assert (abs((_cp_dim3 or 0) - 0.8) < 0.01)
        with subtests.test(msg="AT COPY: red channel copied to fixture 2"):
            assert (_cp_r2 == 200)
        with subtests.test(msg="AT COPY: red channel copied to fixture 3"):
            assert (_cp_r3 == 200)
        with subtests.test(msg="AT COPY: green channel copied to fixture 2"):
            assert (_cp_g2 == 50)
        prog.clear_programmer()
        ...
        with subtests.test(msg="stk NOTE view returns has-no-note message when blank"):
            assert ("no note" in _no_note_msg.lower() or "set with" in _no_note_msg.lower())
        run_command("stk 99 NOTE Dark Moody Show")
        with subtests.test(msg="stk NOTE set stores text"):
            assert (_csnote_cs.note == "Dark Moody Show")
        _note_view = run_command("stk 99 NOTE")
        with subtests.test(msg="stk NOTE view returns the note text"):
            assert ("Dark Moody Show" in _note_view)
        _csnote_cs.note = ""
        ...
        _mk_cue(2, 0.66)
        _mk_cue(3, 1.0)
        with subtests.test(msg="BOUNCE: stack has 3 cues"):
            assert (len(_bcs.cues) == 3)
        run_command(f"stk {_bcs_id} bounce on")
        with subtests.test(msg="stk bounce on sets .bounce = True"):
            assert (_bcs.bounce is True)
        ...
        with subtests.test(msg="BOUNCE GO 1: at cue 1"):
            assert (_bcs.current == 1.0)
        _bcs.go(patch, fade_engine, _bex)   # fires cue 2
        with subtests.test(msg="BOUNCE GO 2: at cue 2"):
            assert (_bcs.current == 2.0)
        _bcs.go(patch, fade_engine, _bex)   # fires cue 3
        with subtests.test(msg="BOUNCE GO 3: at cue 3"):
            assert (_bcs.current == 3.0)
        _bcs.go(patch, fade_engine, _bex)   # hits end → reverses → fires cue 2
        with subtests.test(msg="BOUNCE GO 4: reverses at last cue → cue 2"):
            assert (_bcs.current == 2.0)
        with subtests.test(msg="BOUNCE GO 4: direction flipped to -1"):
            assert (_bcs._bounce_dir == -1)
        _bcs.go(patch, fade_engine, _bex)   # fires cue 1
        with subtests.test(msg="BOUNCE GO 5: at cue 1"):
            assert (_bcs.current == 1.0)
        _bcs.go(patch, fade_engine, _bex)   # hits start → reverses → fires cue 2
        with subtests.test(msg="BOUNCE GO 6: reverses at first cue → cue 2"):
            assert (_bcs.current == 2.0)
        with subtests.test(msg="BOUNCE GO 6: direction flipped to +1"):
            assert (_bcs._bounce_dir == 1)
        run_command(f"stk {_bcs_id} bounce off")
        with subtests.test(msg="stk bounce off sets .bounce = False"):
            assert (_bcs.bounce is False)
        ...
        prog.data.clear()
        _cc.current = 1.0
        with subtests.test(msg="stk CLEAR: setup has 2 cues"):
            assert (len(_cc.cues) == 2)
        _r_clear_cs = run_command(f"stk {_cc_id} CLEAR")
        with subtests.test(msg="stk CLEAR: removes all cues"):
            assert (len(_cc.cues) == 0)
        with subtests.test(msg="stk CLEAR: resets current to None"):
            assert (_cc.current is None)
        with subtests.test(msg="stk CLEAR: returns confirmation"):
            assert ("cleared" in _r_clear_cs.lower())
        ...
        if _ln_cs:
            ...
            with subtests.test(msg="LIST NOTES: includes stack note"):
                assert ("ListNotesTest" in _ln_result)
            if _cue1:
                with subtests.test(msg="LIST NOTES: includes cue note"):
                    assert ("CueNoteTest" in _ln_result)
        ...
        if _fg_fid is not None:
            ...
            if _fg_in_group:
                ...
                with subtests.test(msg="FIXTURE GROUPS: returns group membership info"):
                    assert ("Group" in _fg_result or "group" in _fg_result)
                with subtests.test(msg="FIXTURE GROUPS: mentions the fixture name"):
                    assert (patch.fixtures[_fg_fid].name in _fg_result
                       or str(_fg_fid) in _fg_result)
            # Fixture not in any group scenario
            _fg_not_found = run_command("FIXTURE GROUPS 9999")
            with subtests.test(msg="FIXTURE GROUPS: bad fixture returns error"):
                assert ("not patched" in _fg_not_found.lower())
        ...
        with subtests.test(msg="RENAME FIXTURE: bad ID returns error"):
            assert ("not in patch" in _rf_bad)
        ...
        with subtests.test(msg="PROGRAMMER SCALE: returns confirmation"):
            assert ("50%" in _r_ps or "scaled" in _r_ps.lower())
        _ps_dim = prog.data.get("1", {}).get('dim')
        _ps_r   = prog.data.get("1.1", {}).get('red')
        _ps_g   = prog.data.get("1.1", {}).get('green')
        with subtests.test(msg="PROGRAMMER SCALE 50: dim scaled to 50%"):
            assert (abs((_ps_dim or 0) - 0.5) < 0.01)
        with subtests.test(msg="PROGRAMMER SCALE 50: red 200 → 100"):
            assert (_ps_r == 100)
        with subtests.test(msg="PROGRAMMER SCALE 50: green 100 → 50"):
            assert (_ps_g == 50)
        # Test 200% (amplify + clamp)
        run_command("PROGRAMMER SCALE 200")
        _ps_r2 = prog.data.get("1.1", {}).get('red')
        with subtests.test(msg="PROGRAMMER SCALE 200: red 100 × 2 = 200"):
            assert (_ps_r2 == 200)
        # Test empty programmer
        prog.clear_programmer()
        _r_ps_empty = run_command("PROGRAMMER SCALE 50")
        with subtests.test(msg="PROGRAMMER SCALE: empty programmer returns error"):
            assert ("empty" in _r_ps_empty.lower())
        prog.clear_programmer()
        ...
        if _fa_cs:
            ...
            with subtests.test(msg="FADER ASSIGN stk: wires stack to fader"):
                assert (_fa_ex.stack is _fa_cs)
            with subtests.test(msg="FADER ASSIGN stk: returns confirmation"):
                assert ("stk 1" in _fa_result or "assigned" in _fa_result.lower())
            _r_fa_bad = run_command("FADER 15 ASSIGN stk 9999")
            with subtests.test(msg="FADER ASSIGN stk: bad stk returns error"):
                assert ("not found" in _r_fa_bad.lower())
        ...
        if stack_pool.get(1):
            ...
            with subtests.test(msg="FADER UNASSIGN: setup — stack assigned"):
                assert (_fu_ex.stack is not None)
            _r_ua = run_command("FADER 16 UNASSIGN")
            with subtests.test(msg="FADER UNASSIGN: stack is None after unassign"):
                assert (_fu_ex.stack is None)
            with subtests.test(msg="FADER UNASSIGN: returns confirmation"):
                assert ("unassigned" in _r_ua.lower())
            _r_ua_empty = run_command("FADER 16 UNASSIGN")
            with subtests.test(msg="FADER UNASSIGN: returns error when already empty"):
                assert ("no stack" in _r_ua_empty.lower())
        ...
        with subtests.test(msg="FAN DIM sets fixture 1 to 0%"):
            assert (abs(_fan_dims[0] or 0) < 0.01)
        with subtests.test(msg="FAN DIM sets fixture 6 to 100%"):
            assert (abs((_fan_dims[5] or 0) - 1.0) < 0.01)
        with subtests.test(msg="FAN DIM is monotone across selection"):
            assert (all(_fan_dims[i] is not None and _fan_dims[i] <= _fan_dims[i+1]
                   for i in range(5)))
        run_command("FAN R 0 255")
        _fan_r = [prog.data.get(f"{i}.1", {}).get('red') for i in range(1, 7)]
        with subtests.test(msg="FAN R sets fixture 1 red to 0"):
            assert (_fan_r[0] == 0)
        with subtests.test(msg="FAN R sets fixture 6 red to 255"):
            assert (_fan_r[5] == 255)
        prog.clear_programmer()
        ...
        with subtests.test(msg="RANDOM 3 selects exactly 3 master fixtures"):
            assert (len(_sel_m) == 3)
        prog.clear_programmer()
        run_command("1 THRU 6 EVERY 2")   # selects 1, 3, 5
        _sel_m2 = [m.fixture_id for m in prog.selection if isinstance(m, MasterFixture)]
        with subtests.test(msg="1 THRU 6 EVERY 2 selects 1,3,5"):
            assert (_sel_m2 == [1, 3, 5])
        prog.clear_programmer()
        ...
        if len(_all_ids) >= 2:
            ...
            with subtests.test(msg="NEXT advances selection to next fixture"):
                assert (_sel_masters == [_all_ids[1]])
            run_command("PREV")               # should go back to first
            _sel_masters2 = [m.fixture_id for m in prog.selection
                             if isinstance(m, MasterFixture)]
            with subtests.test(msg="PREV retreats selection to previous fixture"):
                assert (_sel_masters2 == [_all_ids[0]])
            prog.clear_programmer()

        # ── PATCH RENAME / PATCH MOVE TESTS ──────────────────────────────
        _tmp_fix = patch.patch_fixture(51, "TmpFix", "Generic_RGB", 1, 490)
        with subtests.test(msg="PATCH RENAME test fixture patched"):
        ...
        with subtests.test(msg="PATCH MOVE updates sub universe"):
            assert (_first_sub_51 is not None and _first_sub_51.outputs[0]['universe'] == 2)
        with subtests.test(msg="PATCH MOVE updates sub address"):
            assert (_first_sub_51 is not None and _first_sub_51.outputs[0]['address'] == 50)
        ...
        with subtests.test(msg="MACRO RECORD starts recording"):
            assert (_macro_recording["slot"] == 99)
        run_command("AT FULL")          # captured inside macro
        run_command("AT R 200")         # captured inside macro
        r_ms = run_command("MACRO STOP")
        with subtests.test(msg="MACRO STOP saves macro to pool"):
            assert (99 in macro_pool)
        with subtests.test(msg="MACRO STOP records correct command count"):
            assert (len(macro_pool.get(99, {}).get("commands", [])) == 2)
        with subtests.test(msg="MACRO STOP ends recording"):
            assert (_macro_recording["slot"] is None)
        prog.clear_programmer()
        run_command("1")
        r_mb = run_command("MACRO 99")
        with subtests.test(msg="MACRO playback result mentions commands played"):
            assert ("cmd" in r_mb)
        with subtests.test(msg="MACRO playback restores dim"):
            assert (abs(prog.data.get('1', {}).get('dim', 0) - 1.0) < 0.01)
        with subtests.test(msg="MACRO playback restores red channel"):
        ...
        with subtests.test(msg="MACRO self-recursion blocked, not a crash"):
            assert ("blocked" in r_self)
        with subtests.test(msg="MACRO play stack cleaned up after self-recursion block"):
            assert (len(_macro_play_stack) == 0)
        macro_pool[95] = {"name": "A", "commands": ["MACRO 96"]}
        macro_pool[96] = {"name": "B", "commands": ["MACRO 95"]}
        r_cycle = run_command("MACRO 95")
        with subtests.test(msg="MACRO indirect A->B->A cycle blocked, not a crash"):
            assert ("blocked" in r_cycle)
        with subtests.test(msg="MACRO play stack cleaned up after cycle block"):
            assert (len(_macro_play_stack) == 0)
        del macro_pool[97]
        del macro_pool[95]
        del macro_pool[96]
        ...
        with subtests.test(msg="PARK adds fixture to parked_fids"):
            assert (1 in output_state.parked_fids)
        with subtests.test(msg="PARK stores some parked addresses"):
            assert (bool(output_state.parked_addresses))
        ...
        with subtests.test(msg="PARK holds DMX value against programmer change"):
            assert (_park_dmx[_r_addr] == 200)
        # Unpark
        r_unpark = run_command("UNPARK")
        with subtests.test(msg="UNPARK removes fixture from parked_fids"):
            assert (1 not in output_state.parked_fids)
        prog.clear_programmer()
        run_command("1")
        run_command("AT R 0 G 0 B 0")
        _after_dmx = output_state.get_dmx_for_universe(1)
        with subtests.test(msg="After UNPARK, programmer changes take effect"):
            assert (_after_dmx[_r_addr] == 0)
        prog.clear_programmer()
        ...
        try:
            ...
            with subtests.test(msg="PARK still holds its value while FREEZE is active"):
                assert (_frozen_dmx[_park_addr] == 222)
        finally:
        ...
        try:
            ...
            with subtests.test(msg="AI 'dim' action clamps out-of-range value to 1.0"):
                assert (output_state.programmer_layer.get("1", {}).get('dim') == 1.0
                   and patch.get(1).virtual_dimmer == 1.0)
            _test_ai.execute([{"action": "dim", "value": -3.0}])
            with subtests.test(msg="AI 'dim' action clamps negative value to 0.0"):
                assert (output_state.programmer_layer.get("1", {}).get('dim') == 0.0
        ...
        with subtests.test(msg="stk CHASE ON enables chase_enabled"):
            assert (_ch is not None and _ch.chase_enabled is True)
        with subtests.test(msg="stk CHASE ON BPM sets chase_bpm"):
            assert (_ch is not None and abs(_ch.chase_bpm - 120.0) < 0.1)
        with subtests.test(msg="stk CHASE ON returns confirmation"):
            assert ("chase" in r_ch_on.lower())
        r_ch_bpm = run_command("stk 103 CHASE BPM 90")
        with subtests.test(msg="stk CHASE BPM updates chase_bpm"):
            assert (_ch is not None and abs(_ch.chase_bpm - 90.0) < 0.1)
        r_ch_off = run_command("stk 103 CHASE OFF")
        with subtests.test(msg="stk CHASE OFF disables chase_enabled"):
            assert (_ch is not None and _ch.chase_enabled is False)
        with subtests.test(msg="stk CHASE OFF returns confirmation"):
            assert ("chase" in r_ch_off.lower())
        # stk INFO shows chase state
        r_ch_info = run_command("stk 103 INFO")
        with subtests.test(msg="stk INFO shows chase field"):
            assert ("Chase" in r_ch_info or "chase" in r_ch_info.lower())
        ...
        with subtests.test(msg="stk CHASE save/load preserves chase_enabled"):
            assert (_reloaded_ch is not None and _reloaded_ch.chase_enabled is True)
        with subtests.test(msg="stk CHASE save/load preserves chase_bpm"):
            assert (_reloaded_ch is not None and abs(_reloaded_ch.chase_bpm - 77.0) < 0.1)
        ...
        with subtests.test(msg="FADER SIZE+ nudges size_factor up to ~1.25"):
            assert (abs(_sz_ex.size_factor - 1.25) < 0.01)
        with subtests.test(msg="FADER SIZE+ returns confirmation"):
            assert ("size" in r_sz_plus.lower())
        run_command("FADER 3 SIZE-")
        with subtests.test(msg="FADER SIZE- returns size_factor to ~1.00"):
            assert (abs(_sz_ex.size_factor - 1.0) < 0.01)
        run_command("FADER 3 SIZE+")
        r_sz_reset = run_command("FADER 3 SIZE RESET")
        with subtests.test(msg="FADER SIZE RESET returns size_factor to 1.0"):
            assert (_sz_ex.size_factor == 1.0)
        with subtests.test(msg="FADER SIZE RESET returns confirmation"):
            assert ("reset" in r_sz_reset.lower())
        r_sz_set = run_command("FADER 3 SIZE 2.0")
        with subtests.test(msg="FADER SIZE 2.0 sets size_factor to 2.0"):
            assert (abs(_sz_ex.size_factor - 2.0) < 0.01)
        with subtests.test(msg="FADER SIZE 2.0 returns confirmation"):
            assert ("2.0" in r_sz_set or "2.00" in r_sz_set)
        ...
        if _sz_layer_id is not None:
            ...
            with subtests.test(msg="FADER SIZE: _apply_size_factor sets size_scale on owned layer"):
                assert (_sz_layer is None or abs(_sz_layer.size_scale - 0.5) < 0.01)
        _sz_ex.size_factor = 1.0
        _sz_ex._apply_size_factor()
        run_command("FX CLEAR")
        prog.clear_programmer()
        ...
        with subtests.test(msg="AT … IN creates a live_fade entry"):
            assert (len(prog.live_fades) >= 1)
        _fade_entry = next(
            (f for f in prog.live_fades if f['fid'] == '1' and f['channel'] == 'dim'),
            None)
        with subtests.test(msg="live_fade entry has correct dst (0.0)"):
            assert (_fade_entry is not None and abs(_fade_entry['dst'] - 0.0) < 0.01)
        with subtests.test(msg="live_fade entry has correct duration (5.0)"):
            assert (_fade_entry is not None and abs(_fade_entry['duration'] - 5.0) < 0.01)
        with subtests.test(msg="live_fade entry has a non-zero src"):
            assert (_fade_entry is not None and (_fade_entry['src'] or 0) > 0.0)
        # PROG FADE CLEAR should purge all active fades
        r_pfc = run_command("PROG FADE CLEAR")
        with subtests.test(msg="PROG FADE CLEAR empties live_fades list"):
            assert (len(prog.live_fades) == 0)
        with subtests.test(msg="PROG FADE CLEAR returns confirmation"):
            assert ("clear" in r_pfc.lower())
        prog.clear_programmer()
        ...
        with subtests.test(msg="LIST REFS finds the cue"):
            assert ("track test cue" in _r_refs or "cue 90" in _r_refs.lower() or "stk 1" in _r_refs.lower())
        ...
        with subtests.test(msg="UPDATE COLOR returns confirmation"):
            assert ("updated" in _r_upd.lower())
        with subtests.test(msg="UPDATE COLOR live-pushed message"):
            assert ("live-pushed" in _r_upd.lower())
        with subtests.test(msg="Color preset 10 updated"):
            assert (color_pool.get(10) is not None)
        ...
        with subtests.test(msg="FIRE FX tags layers with fx_preset_ref"):
            assert (_fx_entry is not None and any(ld.get('fx_preset_ref') == 11
        ...
        with subtests.test(msg="LIST REFS FX finds the cue"):
            assert ("fx track cue" in _r_fx_refs.lower() or "cue 91" in _r_fx_refs.lower() or "stk 1" in _r_fx_refs.lower())

        # UPDATE FX
        run_command("FX SINE RED BPM 90")
        _r_upd_fx = run_command("UPDATE FX 11")
        with subtests.test(msg="UPDATE FX returns confirmation"):
            assert ("updated" in _r_upd_fx.lower())
        with subtests.test(msg="UPDATE FX live-pushed message"):
            assert ("live-pushed" in _r_upd_fx.lower())
        ...
        with subtests.test(msg="flash_on activates fader"):
            assert (_fe.is_active)
        _saved_pos = _fe_cs.current
        _fe.flash_off()
        with subtests.test(msg="flash_off deactivates fader"):
            assert (not _fe.is_active)
        with subtests.test(msg="flash_off preserves cue position"):
            assert (_fe_cs.current == _saved_pos)
        ...
        with subtests.test(msg="moment fader excluded from active_layers at level 0"):
            assert (all(lyr is not _me.layer for lyr, _ in _active))
        _me.level = 0.5
        _exec_fader_mode_hook(_me)
        with subtests.test(msg="moment fader is_active True when level > 0"):
            assert (_me.is_active)

        # ── Fader output_mode: vfade ──────────────────────────────────────────
        r_vfd = run_command("FADER 97 OUTPUT VFADE")
        with subtests.test(msg="FADER OUTPUT VFADE command works"):
            assert ("vfade" in r_vfd.lower())
        with subtests.test(msg="output_mode set to vfade"):
            assert (_me.output_mode == 'vfade')
        run_command("FADER 97 OUTPUT NORMAL")
        with subtests.test(msg="FADER OUTPUT NORMAL resets mode"):
            assert (_me.output_mode == 'normal')

        # ── trigger_mode moment ───────────────────────────────────────────────
        r_mom = run_command("FADER 97 MODE MOMENT")
        with subtests.test(msg="FADER MODE MOMENT accepted"):
            assert ("moment" in r_mom.lower())
        with subtests.test(msg="trigger_mode set to moment"):
            assert (_me.trigger_mode == 'moment')
        r_offt = run_command("FADER 97 OFFTIME 1.5")
        with subtests.test(msg="FADER OFFTIME sets off_time"):
            assert (abs(_me.off_time - 1.5) < 0.001)
        ...
        _ve.level       = 0.5
        _vfade_apply(_ve)
        with subtests.test(msg="vfade_apply lerps at 0.5"):
            assert (abs(_ve.layer.get("1", {}).get("red", 0) - 127.5) < 0.5)
        _ve.level = 0.0
        _vfade_apply(_ve)
        with subtests.test(msg="vfade_apply at 0 = from state"):
            assert (_ve.layer.get("1", {}).get("red", -1) == 0.0)
        _ve.level = 1.0
        _vfade_apply(_ve)
        with subtests.test(msg="vfade_apply at 1 = to state"):
            assert (abs(_ve.layer.get("1", {}).get("red", 0) - 255.0) < 0.5)

    except Exception as e:
        with subtests.test(msg=f"smoke test raised {type(e).__name__}: {e}"):
            assert (False)
    ...
    if _DPG_OK:
        ...
        try:
            gui.build()
            with subtests.test(msg="gui.build() constructs all windows/widgets without error"):
                assert (True)
            ...
            with subtests.test(msg="fixture-dim slider sets virtual_dimmer to the slider's "
                   "actual fraction, not fraction*100 clamped to 1.0"):
                assert (patch.get(1).virtual_dimmer == 0.5)
            gui._on_fixture_dim_slider(None, 1.0, 1)
            patch.get(1).virtual_dimmer = 1.0  # restore default for any later use
        except Exception as e:
            with subtests.test(msg=f"gui.build() raised {type(e).__name__}: {e}"):
                assert (False)
        finally:
        ...
        with subtests.test(msg="gui.build() constructs all windows/widgets without error (skipped: dearpygui not installed)"):
            assert (True)
